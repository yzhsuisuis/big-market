...
import cn.bugstack.domain.credit.service.adjust.CreditAdjustService;
...
public class RaffleActivityController implements IRaffleActivityService {
    ...
    public Response<Boolean> armory(@RequestParam Long activityId) {
        try {
            ...
            // 2. 策略装配,这里是通过方法重载,
            //本来只需要传递一个strategyId就行的
            //这个里传一个activityId,在方法里面查出来strategyId
    ...
    public Response<ActivityDrawResponseDTO> draw(@RequestBody ActivityDrawRequestDTO request) {
        //ActivityDrawRequestDTO : userId 和 activityId
        try {
            ...
            // 2. 参与活动 - 创建参与记录订单,在这里透传一个activity的endtime
    ...
    public Response<BigDecimal> queryUserCreditAccount(String userId) {
        try {
            log.info("查询用户积分额度开始 :{}",userId);
            CreditAccountEntity creditAccountEntity = creditAdjustService.queryUserCreditAccount(userId);
            log.info("查询用户积分额度完成 userId:{} adjustAmount:{}", userId, creditAccountEntity.getAdjustAmount());
            ...
            log.error("查询用户积分额度失败 userId:{}",userId,e);
            return Response.<BigDecimal>builder()
                    .code(ResponseCode.UN_ERROR.getCode())
                    .info(ResponseCode.UN_ERROR.getInfo())
                    .build();
    ...
    public Response<Boolean> creditPayExchangeSku(@RequestBody SkuProductShopCartRequestDTO request) {
        try {
            ...
            UnpaidActivityOrderEntity unpaidActivityOrder = raffleActivityAccountQuotaService.createOrder(SkuRechargeEntity.builder()
                    ...
                    .build());
            log.info("积分兑换商品结束,创建订单成功 userId:{} sku:{} outBusinessOn :{}",request.getUserId(),request.getSku(),unpaidActivityOrder.getOutBusinessNo());
